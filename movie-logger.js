window.onload = () => {
    const addMovieButton = document.getElementById("addMoviesButton")
    ...
    const movies = document.getElementById("moviesDiv")
    ...
    if(storedMoviesFromStorage != null) {
        ...
        Object.keys(storedMoviesJSON).forEach(key => {
            ...
            if(movie.movieImage != null && movie.movieName != null) {
                ...
                <p>Rating given: ${movie.movieRating}/10</p>
                <p>${movie.movieReview}</p>
                <div style="text-align: center;">
                    <button class="btn addMovieBTN"  onclick="deleteMovie('${movie.movieName}')">Delete Movie</button>
                </div>
            </div>`
            }  
        })
        ...
        .then(async (response) => {
            ...
            searchMovies.forEach(movie => {
                if(movie.poster_path != null && movie.title != null) {
                    ...
                    <div style="text-align: center;">
                        <button class="btn addMovieBTN" onclick="addMovie('${movie.title}', '${movie.poster_path}')">Add Movie</button>
                    </div>
                    </div>`
                }  
            })
